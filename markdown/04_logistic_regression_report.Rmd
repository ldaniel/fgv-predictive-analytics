...
We will keep only dummies that has the event in at least 5% of the observation in the dataset.

Below table shows the dummy variables that will be kept in the model:
...
That’s why we will really on a test called Variance Inflation Factors, VII for short, to detect multicollinearity.

Multicollinearity is poison for Regression algorithms!!! :skull: `:skull:`:skull: `:skull:`:skull: `:skull:`
...
Regression algorithms are known for its sensitivity for outliers.

So, before we go for the modeling juice we will look for those bastards!!!
...
With the final cleaned dataset, we got from above steps we fit our Logistic Regression model for y_loan_defaulter on all x variables.
...
Below functions were developed in this exercise to provide that.
...
Below the confusion matrix and general performance of the model using our custom function developed in this exercise:
