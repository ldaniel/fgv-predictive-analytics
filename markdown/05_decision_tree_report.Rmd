...
Pruning is a technique in machine learning used to reduce the size of decision tree by removing nodes that provide little power of classification.
...
- **x_loan_amount**
- **x_no_of_commited_crimes_195**

The result is not so different from the Logistic Regression model we created in the session before.
...
We finally look at the score distribution charts how well the model is able to discriminate Defaulters and Non-Defaulters
...
The box plot also does not show a clear separation between Defaulters and Non-Defaulters.
