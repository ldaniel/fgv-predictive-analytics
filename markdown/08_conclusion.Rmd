...
Here we will expand the analysis done individual in each model to compare how it is performing against each other.
...
We will use a custom function developed in this exercise to calculate the best cut off for each model:
See function description in the Data Preparation Session of this site.

With the returned object from these functions we can plot the comparison between TPR (True Positive Rate) and FPR (False Positive Rate) to find the best cut off.
...
Having the best score cut off of each model we use **HMeasure()** function from **hmeasure** library to calculate the full set of metrics for classification methods.
...
Interesting to notice here how narrow the scores produced from Decision Tree model compared to the other models.

As discussed in the Decision Tre session this model is extremely limited for this dataset with a huge variance in the tree depending on the train data and hyper parameters tuning.
...
Here we can see that Random Forest has an edge on the classification power against the other models.

This is the only model where the box plot interquartile does not overlap.
...
```{r ROC, echo=TRUE, out.width= '100%'}
Plot_ROC(prob.test, smooth_opt = FALSE)
```

### Acurracy

We finally look at the Confusion Matrix at the best cut off each model to get a sense of the accuracy we were able to get in this exercise.

Random Forest is the best model we got in this exercise.
...
Unfortunately, this is not a real dataset with really interest variables client income, credit rate, and so on.
